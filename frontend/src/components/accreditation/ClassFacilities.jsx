...
const ClassFacilities = ({ onStepSubmit }) => {
  ...
  const fetchFacilities = useCallback(async () => {
    ...
    try {
      ...
      setError('خطا در دریافت اطلاعات تجهیزات صنفی');
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const validateForm = () => {
    ...
    if (!formData.equipment_name.trim()) {
      setError('لطفاً نام وسیله درسی را وارد کنید');
      return false;
    }
  ...
  const handleAddEntry = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'خطا در افزودن تجهیزات صنفی');
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError('Error deleting facility data');
    }
  };
      ...
      <div className="form-container">
        <div className="form-header">
          <h3 className="form-title">
            ...
            تجهیزات صنفی مرکز آموزشی
          </h3>
          <p className="form-description" style={theme === 'light' ? { color: '#00b5d7' } : {  color: '#a9e5ff' }}>
            میزان موجودیت تجهیزات و مواد ممد درسی از قبیل کتب، میز، چوکی مناسب پروجکتور یا LCD در صنوف را بصورت مشرح بیان داشته و طی جدول بصورت رشته وار لست نموده و کیفیت آنرا نیز بیان دارید.
          </p>
        </div>
        {success && (
...
ClassFacilities.propTypes = {
  onStepSubmit: PropTypes.func,
};

ClassFacilities.defaultProps = {
  onStepSubmit: () => {},
};
