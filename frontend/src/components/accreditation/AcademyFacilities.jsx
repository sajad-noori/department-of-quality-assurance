...
const AcademyFacilities = ({ onStepSubmit }) => {
  ...
  const fetchFacilities = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/api/academy-facilities`, {
      ...
      setError('خطا در دریافت اطلاعات امکانات');
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const handleAddEntry = async () => {
    ...
    setSuccess(null);
    try {
      const response = await axios.post(`${API_BASE_URL}/api/academy-facilities`, formData, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
      ...
      setError(err.response?.data?.message || 'خطا در افزودن امکانات');
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`${API_BASE_URL}/api/academy-facilities/${id}`, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
  ...
  return (
    <div className="academy-facilities-form glass-bg" dir="rtl">
      <div className="form-container">
        <div className="form-header">
          ...
          <p className="form-description"    style={theme === 'light' ? { color: '#00b5d7' } : {  color: '#a9e5ff' }}
              >
            در این بخش تمامی امکانات اساسی و اولیه مورد نیاز از قبیل ساختمان، صنوف، ورکشاپ، لابراتوار، فارم تحقیقاتی و لیلیه را با جزئیات که در جدول ذیل ذکر گردیده لست نمائید.
          </p>
        </div>
        {success && (
        ...
        <fieldset className="mb-3 border rounded p-2 form-section">
          <legend className="float-none w-auto px-2 mb-2 small">امکانات مرکز آموزشی</legend>
          <div className="row g-3">
            ...
            <div className="col-12 col-md-6">
              <input
                ...
                onChange={handleChange}
                onFocus={() => handleFocus('basic_facilities')}
                onBlur={handleBlur}
                className={`form-control white-placeholder ${focusedField === 'basic_facilities' ? 'focused' : ''}`}
                style={theme === 'light' ? { background: '#fff', color: '#222' } : { background: 'transparent', color: 'white' }}
                disabled={isSubmitting}
              />
            </div>
      ...
      <style>{`
        ...
        .section-title {
          font-size: clamp(1rem, 1.6vw + 0.3rem, 1.2rem);
          font-weight: 600;
          color: #0dcaf0;
          margin: 0;
          display: flex;
...
AcademyFacilities.propTypes = {
  onStepSubmit: PropTypes.func,
};

AcademyFacilities.defaultProps = {
  onStepSubmit: () => {},
};
