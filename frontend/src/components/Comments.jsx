...
const Comments = ({ newsId }) => {
  ...
  useEffect(() => {
    if (location.state && location.state.scrollToCommentId) {
      ...
      setTimeout(() => setHighlightedId(null), 3400);
    }
  ...
  return (
    <div style={isLight ? containerStyleLight : containerStyle}>
      ...
      {error && (
        <div style={isLight ? errorStyleLight : errorStyle}>❌ {error}</div>
      )}

      {comments.length === 0 && !loading && (
        <div style={isLight ? emptyStateStyleLight : emptyStateStyle}>
          <div style={isLight ? emptyStateIconStyleLight : emptyStateIconStyle}>
            💭
          </div>
      ...
      <ul style={isLight ? commentsListStyleLight : commentsListStyle}>
        {currentComments.map((c) => (
          <li
            ...
            style={{
              ...(isLight ? commentItemStyleLight : commentItemStyle),
              ...(highlightedId === c.id
                ? isLight
                  ? {
                  ...
                  : {
                      border: "2.5px solid #ffe066", // bright yellow border for dark mode
                      boxShadow: "0 0 0 4px #ffe066",
                      background: "#333300", // dark yellow background for dark mode
                    }
                : {}),
            }}
            onMouseEnter={(e) =>
              Object.assign(
                ...
                isLight ? commentItemHoverStyleLight : commentItemHoverStyle
              )
            }
            onMouseLeave={(e) =>
              Object.assign(
            ...
            {replyForms.has(c.id) && (
              <div
                style={{
                  marginTop: "1rem",
                  padding: "1rem",
                  backgroundColor: isLight ? "#f0f8ff" : "#1a1a1a",
                  borderRadius: "6px",
              ...
              >
                ...
                >
                  <button
                    ...
                    disabled={
                      submittingReplies.has(c.id) || !replyTexts[c.id]?.trim()
                    }
                    style={{
                      backgroundColor:
                        submittingReplies.has(c.id) || !replyTexts[c.id]?.trim()
            ...
            {expandedReplies.has(c.id) && replies[c.id] && (
              <div
                style={{
                  marginTop: "1rem",
                  paddingLeft: "1rem",
                  borderLeft: `2px solid ${isLight ? "#e0f7fa" : "#333"}`,
                }}
              >
      ...
      {totalComments > 0 && (
        <div
          style={
            isLight ? paginationContainerStyleLight : paginationContainerStyle
          }
        >
          <div style={isLight ? paginationInfoStyleLight : paginationInfoStyle}>
            نمایش {startIndex + 1} تا {Math.min(endIndex, totalComments)} از{" "}
            {totalComments} نظر
          </div>

          <div
            style={
              isLight ? paginationControlsStyleLight : paginationControlsStyle
            }
          >
      ...
      <div style={isLight ? formContainerStyleLight : formContainerStyle}>
        {!user && (
          <div style={isLight ? loginPromptStyleLight : loginPromptStyle}>
            <p
              style={isLight ? loginPromptTextStyleLight : loginPromptTextStyle}
            >
            ...
            <button
              ...
              onMouseEnter={(e) =>
                Object.assign(
                  e.currentTarget.style,
                  isLight ? loginButtonHoverStyleLight : loginButtonHoverStyle
                )
              }
              onMouseLeave={(e) =>
                Object.assign(
                  e.currentTarget.style,
                  isLight ? loginButtonStyleLight : loginButtonStyle
                )
              }
            >
              ورود به سیستم
        ...
        <button
          ...
          style={
            submitting || !user || !newComment.trim()
              ? isLight
                ? buttonDisabledStyleLight
                : buttonDisabledStyle
              : isLight
              ? buttonStyleLight
              : buttonStyle
