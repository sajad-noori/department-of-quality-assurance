const PracticalFacility = require('../models/practical_facility.model');

const practicalFacilityController = {
  async createFacility(req, res) {
    try {
      ...
      const facilityId = await PracticalFacility.create({
        user_id,
        name,
        equipment_name,
        equipment_count,
  ...
  async getFacilities(req, res) {
    try {
      const user_id = req.user.id;
      const facilities = await PracticalFacility.findByUserId(user_id);
  ...
  async deleteFacility(req, res) {
    try {
      ...
      const deleted = await PracticalFacility.delete(id, user_id);
...
module.exports = practicalFacilityController; 