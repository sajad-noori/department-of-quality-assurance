    ...
    try {
      ...
      if (!validTypes.includes(data.file_type)) {
        throw new Error(
          index < 3
            ? "Invalid file type"
            : "Only zip files are allowed for this field"
        );
      }
      ...
      const [existingRows] = await promise.execute(checkQuery, [data.user_id]);
      ...
      if (existingRows.length === 0) {
        ...
        const insertQuery = `
          ...
          VALUES (?, ?)
        `;
        [result] = await promise.execute(insertQuery, [
          data.user_id,
          data.file_path,
        ]);
      } else {
        ...
        const updateQuery = `
          ...
          WHERE user_id = ?
        `;
        [result] = await promise.execute(updateQuery, [
          data.file_path,
          data.user_id,
        ]);
      }
