...
const sendEmail = async (to, subject, body) => {
  try {
    ...
    // Allow users to paste Google App Passwords grouped with spaces
    // (e.g. "abcd efgh ijkl mnop"). Trim whitespace automatically to
    // avoid authentication errors caused by accidental spaces.
    const rawPass = process.env.EMAIL_PASSWORD || "";
    if (/\s/.test(rawPass)) {
      console.warn(
        "WARNING: EMAIL_PASSWORD contains whitespace; it will be removed before use."
      );
    }
    const emailPassword = rawPass.replace(/\s+/g, "");
